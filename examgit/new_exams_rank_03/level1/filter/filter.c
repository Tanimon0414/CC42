...
int	main(int ac, char **av)
{
	...
	buff = (char *)malloc(MAX_INPUT_BUFFER_SIZE);
	...
	if (buff == NULL)
	{
		...
		perror("");                 // Print the system's error message (e.g.,
									// "Cannot allocate memory").
		return (1);
		// Return 1 as specified for malloc errors.
	}
	char *search_str = av[1]; // Pointer to the search string (e.g.,
								// "bonjour")
								// Temporary storage for a single character read from stdin
								// Stores the return value of read() (number of bytes read,
								// 0 for EOF,
	// -1 for error)
	buff_idx = 0;
	// Index to track the current position in 'buff' while reading
	...
	// an error occurs (bytes_read_result == -1),
	...
	while ((bytes_read_result = read(STDIN_FILENO, &current_char, 1)) > 0)
	{
		...
		//
		// - 1 ensures we always have space for the null terminator.
		...
		// Store the character in 'buff' and increment index.
	}
	// --- Read Error Handling (IMPORTANT) ---
	// After the reading loop,
	// check if the last read operation resulted in an error.
	if (bytes_read_result == -1)
	{
		...
		return (1);
		// Return 1 as specified for read errors.
	}
	...
	process_idx = 0;
	// Index for iterating through 'buff' for processing and output.
	while (buff[process_idx] != '\0')
	{
		// Loop until the null terminator is reached (end of valid input).
		...
		//    for a full 'search_str' match. (process_idx
		// + search_len <= buff_idx)
