...
{
	int c;                       
		// variable to hold the character read using fgetc()
	while ((c = fgetc(f)) != EOF) // Read a character from the stream till EOF
	{
		if (!isspace(c)) // If it's not a whitespace character
		{
			...
			return (0);    
				// Successfully consumed leading whitespace (or found none)
		}
	}
	if (ferror(f)) // loop terminated due to EOF triggered, check read errors
		return (-1);
	// If it's just EOF and no error,
		we've successfully consumed all possible spaces
	return (0);
}
...
int	scan_char(FILE *f, va_list ap)
{
	...
	if (c == EOF)
	{
		if (ferror(f)) // check for read errors
			return (-1);
		return (0); // If it's just EOF and no error,
			means no character available to read
	}
...
/* YOU NEED TO DECLARE,
	INITIALISE AND CLEAN UP THE VA_LIST IN THE BELOW FUNCTION */
