...
char	*get_next_line(int fd)
{
	...
	while (!ft_strchr(stash, '\n'))
	{
		...
		if (!stash) // malloc failed in strjoin,
			stash has already been freed there
			return (NULL);
	}
	...
	leftover_len = ft_strlen(stash + line_length);
	leftover = malloc(sizeof(char) * (leftover_len + 1));
	if (!leftover)
	{
		free(stash);
		stash = NULL;
	...
	leftover[i] = '\0';
	// free original stash,
		reassign the stash pointer to point to leftover bytes
	free(stash);
	stash = leftover;
	return line;
}
